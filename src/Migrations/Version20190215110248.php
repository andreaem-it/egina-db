 ...
 */
final class Version20190215110248 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__guests AS SELECT id, type, location FROM guests');
        $this->addSql('DROP TABLE guests');
        $this->addSql('CREATE TABLE guests (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, name_id_id INTEGER DEFAULT NULL, type INTEGER NOT NULL, location VARCHAR(255) NOT NULL COLLATE BINARY, CONSTRAINT FK_4D11BCB2B162CC12 FOREIGN KEY (name_id_id) REFERENCES articoli (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO guests (id, type, location) SELECT id, type, location FROM __temp__guests');
        $this->addSql('DROP TABLE __temp__guests');
        $this->addSql('CREATE INDEX IDX_4D11BCB2B162CC12 ON guests (name_id_id)');
    }
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('DROP INDEX IDX_4D11BCB2B162CC12');
        $this->addSql('CREATE TEMPORARY TABLE __temp__guests AS SELECT id, type, location FROM guests');
        $this->addSql('DROP TABLE guests');
        $this->addSql('CREATE TABLE guests (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, type INTEGER NOT NULL, location VARCHAR(255) NOT NULL, name VARCHAR(255) NOT NULL COLLATE BINARY)');
        $this->addSql('INSERT INTO guests (id, type, location) SELECT id, type, location FROM __temp__guests');
        $this->addSql('DROP TABLE __temp__guests');
        $this->addSql('DROP INDEX UNIQ_F12DF054A76ED395');
        $this->addSql('CREATE TEMPORARY TABLE __temp__prenotazioni AS SELECT id, user_id, date, motivation, item FROM prenotazioni');
